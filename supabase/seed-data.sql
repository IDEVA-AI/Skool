-- =============================================================
-- iSkool Seed Data
-- Execute no SQL Editor do Supabase Dashboard (bypassa RLS)
...
DELETE FROM auth.users WHERE email IN ('admin@iskool.com.br', 'maria@teste.com', 'joao@teste.com');

-- =============================================================
-- 1. Auth Users + Public Users
-- =============================================================
-- Senha "Teste123!" encriptada com bcrypt
-- Hash gerado via: crypt('Teste123!', gen_salt('bf'))
...
INSERT INTO auth.users (
  ...
  (v_admin_id, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
   'admin@iskool.com.br', v_password_hash,
   v_now, '{"provider":"email","providers":["email"]}', '{"name":"Admin iSkool"}',
   v_now, v_now, '', '', '', ''),
  (v_maria_id, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
   'maria@teste.com', v_password_hash,
   v_now, '{"provider":"email","providers":["email"]}', '{"name":"Maria Silva"}',
   v_now, v_now, '', '', '', ''),
  (v_joao_id, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
   'joao@teste.com', v_password_hash,
   v_now, '{"provider":"email","providers":["email"]}', '{"name":"João Santos"}',
   v_now, v_now, '', '', '', '');
...
-- Reset sequence
SELECT setval('courses_id_seq', 2);
...
INSERT INTO modules (id, course_id, title, description, "order", created_at, updated_at) VALUES
  (1, 1, 'Fundamentos do HTML e CSS', 'Aprenda as bases da web: estrutura HTML semântica e estilização com CSS moderno.', 1, v_now, v_now),
  (2, 1, 'JavaScript Essencial', 'Domine os fundamentos do JavaScript: variáveis, funções, DOM e eventos.', 2, v_now, v_now),
  (3, 1, 'React na Prática', 'Construa aplicações modernas com React, hooks e gerenciamento de estado.', 3, v_now, v_now);

-- Curso 2: Marketing Digital
INSERT INTO modules (id, course_id, title, description, "order", created_at, updated_at) VALUES
  (4, 2, 'SEO e Tráfego Orgânico', 'Estratégias para ranquear no Google e gerar tráfego qualificado sem investir em anúncios.', 1, v_now, v_now),
  (5, 2, 'Tráfego Pago e Anúncios', 'Facebook Ads, Google Ads e estratégias avançadas de mídia paga.', 2, v_now, v_now);

SELECT setval('modules_id_seq', 5);

-- =============================================================
-- 6. Lessons
-- =============================================================
-- Módulo 1: HTML e CSS
INSERT INTO lessons (id, module_id, title, content, video_embed_url, "order", created_at, updated_at) VALUES
  (1, 1, 'Introdução ao HTML',
   'Nesta aula você vai aprender o que é HTML, como funciona a estrutura de uma página web e as principais tags semânticas.',
   'https://www.youtube.com/embed/Ejkb_YpuHWs', 1, v_now, v_now),
  (2, 1, 'Estilizando com CSS',
   'Aprenda a estilizar suas páginas com CSS: seletores, box model, flexbox e grid layout.',
   'https://www.youtube.com/embed/1PnVor36_40', 2, v_now, v_now),
  (3, 1, 'Projeto: Landing Page',
   'Coloque em prática o que aprendeu criando uma landing page completa e responsiva.',
   NULL, 3, v_now, v_now);

-- Módulo 2: JavaScript
INSERT INTO lessons (id, module_id, title, content, video_embed_url, "order", created_at, updated_at) VALUES
  (4, 2, 'Variáveis e Tipos de Dados',
   'Entenda como o JavaScript lida com variáveis (let, const, var) e os diferentes tipos de dados.',
   'https://www.youtube.com/embed/i6Oi-YtXnAU', 1, v_now, v_now),
  (5, 2, 'Funções e Escopo',
   'Aprenda a criar funções, entenda closures e como funciona o escopo em JavaScript.',
   'https://www.youtube.com/embed/hRJrp17WnOE', 2, v_now, v_now);

-- Módulo 3: React
INSERT INTO lessons (id, module_id, title, content, video_embed_url, "order", created_at, updated_at) VALUES
  (6, 3, 'Componentes e JSX',
   'Entenda o conceito de componentes no React, JSX e como criar interfaces declarativas.',
   'https://www.youtube.com/embed/pKYhOoGFz8M', 1, v_now, v_now),
  (7, 3, 'Hooks: useState e useEffect',
   'Domine os dois hooks mais importantes do React para gerenciar estado e efeitos colaterais.',
   'https://www.youtube.com/embed/dpw9EHDh2bM', 2, v_now, v_now);

-- Módulo 4: SEO
INSERT INTO lessons (id, module_id, title, content, video_embed_url, "order", created_at, updated_at) VALUES
  (8, 4, 'Fundamentos de SEO',
   'O que é SEO, como o Google funciona e as principais estratégias de otimização on-page e off-page.',
   'https://www.youtube.com/embed/DvwS7cV9GmQ', 1, v_now, v_now),
  (9, 4, 'Pesquisa de Palavras-Chave',
   'Aprenda a encontrar as melhores palavras-chave para seu nicho usando ferramentas gratuitas e pagas.',
   NULL, 2, v_now, v_now);

-- Módulo 5: Tráfego Pago
INSERT INTO lessons (id, module_id, title, content, video_embed_url, "order", created_at, updated_at) VALUES
  (10, 5, 'Facebook Ads do Zero',
   'Configure sua primeira campanha no Facebook Ads: públicos, criativos e otimização.',
   'https://www.youtube.com/embed/DAQNHzOcO5A', 1, v_now, v_now),
  (11, 5, 'Google Ads para Iniciantes',
   'Aprenda a criar campanhas de pesquisa e display no Google Ads com orçamento reduzido.',
   NULL, 2, v_now, v_now);

SELECT setval('lessons_id_seq', 11);

-- =============================================================
-- 7. Enrollments
-- =============================================================
INSERT INTO enrollments (id, user_id, course_id, enrolled_at) VALUES
  (gen_random_uuid(), v_maria_id, 1, v_now),
  (gen_random_uuid(), v_maria_id, 2, v_now),
  (gen_random_uuid(), v_joao_id,  1, v_now);

-- =============================================================
-- 8. Posts (Feed)
-- =============================================================
INSERT INTO posts (id, community_id, user_id, title, content, pinned, created_at, updated_at) VALUES
  (1, v_community_id, v_admin_id,
...
-- 10. Announcement
-- =============================================================
INSERT INTO announcements (id, community_id, title, content, button_text, button_url, created_at, updated_at) VALUES
  (1, v_community_id,
   'Bem-vindo ao iSkool!',
